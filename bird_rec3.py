...
def highlight_birds(img_path, model, method_name, iteration):
    ...
    if bird_detected or unidentified_detected:
        ...
        output_path = f"highlighted_birds_{method_name}_iteration_{iteration}.jpg"
        cv2.imwrite(output_path, highlighted_img)
        print(f"Highlighted image saved as {output_path}")
    else:
        print(f"No birds detected with method {method_name}, skipping image highlighting.")

def try_all_methods(img_path):
    """Try multiple methods and print results for each."""
    methods = ["InceptionV3"]

    for i, method in enumerate(methods):
        print(f"Trying method: {method} (Iteration {i+1})")
        print("Loading model...")
        model = load_model()
        ...
        print(f"Results for {method} (Iteration {i+1}):")
        if bird_count > 0:
        ...
        highlight_birds(img_path, model, method, i+1)

    print("Finished trying all methods.")
